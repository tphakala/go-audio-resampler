...
func newHalfBandStage(ratio float64, filterLength int) pipeline.Stage {
	...
	stage, err := newPolyphaseStage(ratio, filterLength, halfBandFactor, 0)
	if err != nil {
...
func newPolyphaseStage(ratio float64, _, _, _ int) (pipeline.Stage, error) {
	// Map precision to engine quality
	// Using high quality by default for polyphase stages
...
func newFFTStage(ratio float64, fftSize, _ int) (pipeline.Stage, error) {
	...
	return newPolyphaseStage(ratio, fftSize, defaultFFTPhases, 0)
}
